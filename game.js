...
class Ship {
  ...
  update(dt) {
    ...
    if ((pressed('ShiftLeft') || pressed('ShiftRight')) &&
        this.shieldCooldown <= 0 && this.shieldTime <= 0) {
      this.shieldTime     = SHIELD_DURATION;
      this.shieldCooldown = SHIELD_COOLDOWN;
    }

    const ROT   = 3.5;   // rad/s
    const THRUST = 260 * (this.speedBoost > 0 ? 2 : 1);  // px/s² (x2 con Velocidad)
    const DRAG   = 0.987;
  ...
  draw() {
    ...
    // Escudo: círculo alrededor de la nave
...
// ── Powerups ──────────────────────────────────────────────────────────────────
const POWERUPS = {
  V: { color: '#4ff', label: 'V' },   // Velocidad
  T: { color: '#f4f', label: 'T' },   // Triple disparo
};
...
class Powerup {
  constructor(x, y, kind = 'V') {
    ...
    this.kind   = kind;
    this.radius = 10;
    this.ttl    = 10;   // desaparece si no se recoge
    this.dead   = false;
  }
  ...
  draw() {
    ...
    const { color, label } = POWERUPS[this.kind];
    ctx.save();
    ctx.translate(this.x, this.y);
...
function spawnPowerup() {
  ...
  powerups.push(new Powerup(x, y, Math.random() < 0.5 ? 'V' : 'T'));
}
...
function update(dt) {
  ...
  powerupTimer -= dt;
  if (powerupTimer <= 0 && powerups.length < 1) {
    spawnPowerup();
  ...
  // Nave vs asteroide. El escudo absorbe el impacto y rompe el asteroide.
  // Hoy no hay balas enemigas; si se añaden, reusar este mismo chequeo.
  if (ship.invincible <= 0) {
    ...
    asteroids = asteroids.filter(a => !a.dead).concat(splitAsteroids);
  }
